...
class TBstructure:
    def __init__(self, poscar, atomnames, orbs):
        ...
        for i in range(3):
            self.vec[i] = array([float(s) for s in fi.readline().split()])
        ...
        self.atomname = []
        if (
            str(line1.split()[0])[0] == "D"
            or str(line1.split()[0])[0] == "C"
            or str(line1.split()[0])[0] == "d"
            or str(line1.split()[0])[0] == "c"
        ):
            print "INPUT atom name info in POSCAR!!"
            exit()  # self.specpoint=array([int(s) for s in line0.split()]); linenum+=1
        elif (
            str(line2.split()[0])[0] == "D"
            or str(line2.split()[0])[0] == "C"
            or str(line2.split()[0])[0] == "d"
            or str(line2.split()[0])[0] == "c"
        ):
            self.specpoint = array([int(s) for s in line1.split()])
            self.atomname = [s for s in line0.split()]
        if linenum < sum(self.specpoint):
        ...
        for i in range(sum(self.specpoint)):
            self.at[i] = array([float(s) for s in lines[i + 3].split()[:3]])
        if self.coord == "c":
            self.atc = self.at * self.scale
            ...
            print "ERROR: coordinates must be direct or cartesian"
            sys.exit()
        ...
        for ia, atname in enumerate(atomnames):
            ...
            for ispec, nspec in enumerate(self.specpoint):
                if self.atomname[ispec] == atname:
                    ...
                    idxidx += nspec
            if chkpoint == 0:
                print "Couldn't find the atom name", atname, "in POSCAR"
                exit()
    ...
    def Compute_cor_idx(self, cor_at, cor_orb):
        if len(cor_at) != len(cor_orb):
            print "The number of cor_at should be same as the number of cor_orb"
            exit()
        self.ncor_orb = 0
        self.ncor_orb_list = []
        self.max_cor_orb = 0
        cor_at_flat = [item for sublist in cor_at for item in sublist]
        for at in cor_at_flat:
            self.ncor_orb += len(self.TB_orbs[at])
            self.max_cor_orb = max(self.max_cor_orb, len(self.TB_orbs[at]))
        for at in cor_at:
            self.ncor_orb_list.append(len(self.TB_orbs[at[0]]))
        cor_orb_flat = []
        ...
        for i, ats in enumerate(cor_at):
            ...
            for at in ats:
                for orb in self.TB_orbs[at]:
                    idx = self.idx[at][orb]
                    if (
                        self.orb_idx["s"].count(orb) == 1
                        or self.orb_idx["p"].count(orb) == 1
                    ):
                        print at, " atom ", orb, " orbital is treated by LDA!"
                        self.cor_idx[idx] = 0
                    elif cor_orb_flat[i].count(orb) == 0 and (
                        self.orb_idx["d"].count(orb) == 1
                        or self.orb_idx["f"].count(orb) == 1
                    ):
                        if rank == 0:
                            print at, " atom ", orb, " orbital is treated by HF!"
                        self.cor_idx[idx] = 1
                        self.LHF = ".TRUE."
                        # Nd_at+=dm[idx]
                    elif cor_orb_flat[i].count(orb) == 1 and (
                        self.orb_idx["d"].count(orb) == 1
                        or self.orb_idx["f"].count(orb) == 1
                    ):
                        if rank == 0:
                            print at, " atom ", orb, " orbital is treated by DMFT!"
                        ...
                        print at, " atom ", orb, " orbital is not supported!"
                        exit()
            idx1 += len(cor_orb[i])
