...
class WANNIER:
   def __init__(self,seedname):
      self.name=seedname
   def load_deig(self):
      if os.path.exists(self.name+'.deig'): print "deig file exists!"
      else: print "deig file does not exist! We are exiting"; exit()
      fi_eig=open(self.name+'.deig','r')
      deigvals=[float(line.split()[2]) for line in fi_eig.readlines()] 
      self.num_kpts=int(line.split()[1]);self.num_bands=int(line.split()[0])
      self.deigvals=array(deigvals).reshape(self.num_kpts,self.num_bands)
      fi_eig.close()
...
if __name__=='__main__':
   ...
   WAN.load_chk('/export/home/hyowon/Codes/WANDMFT/bin/')
   WAN.load_eig()
   WAN.Check_Unitarity() 
   print WAN.ikpt
