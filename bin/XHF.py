...
def HF_eigsystem(KPTS,TOT_NKPTS,NBANDMAX,EIGVAL,BAND_WIN,WANU,SigMdc,SigMdc_dn):
   """ This function perform the k-sum of HF Self-consistent calculation
   """
   evalHF=zeros((len(KPTS),NBANDMAX),dtype=float)
   evecHF=zeros((len(KPTS),NBANDMAX,NBANDMAX),dtype=complex)
   evalHF_dn=zeros((len(KPTS),NBANDMAX),dtype=float)
   evecHF_dn=zeros((len(KPTS),NBANDMAX,NBANDMAX),dtype=complex)
   ncor_orb=len(SigMdc)
   ...
   for ikp,kp in enumerate(KPTS):
      if kp<TOT_NKPTS+1: 
         ...
         Eks=diag(EIGVAL[ikp][nbmin:nbmax+1])
         Uwanband=matrix(WANU[ikp][:ncor_orb,:num_band_max])
         Eks_Sigma=Uwanband.H*diag(SigMdc)*Uwanband+matrix(Eks)
         eval,evec=linalg.eigh(Eks_Sigma)
         evalHF[ikp,:num_band_max]=array(eval)
         evecHF[ikp,:num_band_max,:num_band_max]=array(evec)
         Eks_Sigma=Uwanband.H*diag(SigMdc_dn)*Uwanband+matrix(Eks)
         eval,evec=linalg.eigh(Eks_Sigma)
         evalHF_dn[ikp,:num_band_max]=array(eval)
         evecHF_dn[ikp,:num_band_max,:num_band_max]=array(evec)
   return evalHF, evecHF, evalHF_dn, evecHF_dn
   

def Ksum_HF(KPTS,TOT_NKPTS,NBANDMAX,NWANN,EIGVAL,BAND_WIN,WANU,evalHF,evecHF,KWEIGHT,evalHF_dn,evecHF_dn,KWEIGHT_dn):
   """Compute dm and EKIN"""
   DM=zeros(NWANN,dtype=float)
   MOM=zeros(NWANN,dtype=float)
   EKIN=0.0
   for ikp,kp in enumerate(KPTS):
      if kp<TOT_NKPTS+1:
         nbmin=BAND_WIN[ikp][0]; nbmax=BAND_WIN[ikp][1]
         ...
         Uwanband2=Uwanband.H*Uwanband
         DMFTW_perk=matrix(evecHF[ikp][:num_band_max,:num_band_max])*diag(KWEIGHT[ikp][:num_band_max])*matrix(evecHF[ikp][:num_band_max,:num_band_max]).H
         EKIN+=dot(EIGVAL[ikp][nbmin:nbmax+1],diag(DMFTW_perk)).real
         #EKIN+=dot(EIGVAL[ikp][nbmin:nbmax+1],diag(Uwanband2*DMFTW_perk*Uwanband2.H)).real
         DM+=diag(Uwanband*DMFTW_perk*Uwanband.H).real
         MOM+=diag(Uwanband*DMFTW_perk*Uwanband.H).real
         DMFTW_perk=matrix(evecHF_dn[ikp][:num_band_max,:num_band_max])*diag(KWEIGHT_dn[ikp][:num_band_max])*matrix(evecHF_dn[ikp][:num_band_max,:num_band_max]).H
         EKIN+=dot(EIGVAL[ikp][nbmin:nbmax+1],diag(DMFTW_perk)).real
         DM+=diag(Uwanband*DMFTW_perk*Uwanband.H).real
         MOM-=diag(Uwanband*DMFTW_perk*Uwanband.H).real
   return DM,MOM,EKIN       
...
if __name__ == '__main__':
   ...
   else: print "SigMdc_dn.out is missing"; exit()
   ...
   if (os.path.exists('ksum.input')):
      ...
      atm_idx=array([int(elem) for elem in fi.readline().split()])
      U=array([float(elem) for elem in fi.readline().split()])
      Up=array([float(elem) for elem in fi.readline().split()])
      Jh=array([float(elem) for elem in fi.readline().split()])
   else: print "ksum.input is missing"; exit()
   ...
   if rank==0:
      ...
      dEIGVAL_send=zeros((len(KPTS_send),NBANDS),dtype=float)
      dEIGVAL_send[:TOT_NKPTS,:]=array(WAN.deigvals[:,:])
      ...
      dEIGVAL_send=None; EIGVAL_send=None; WANU_send=None; KWEIGHT_send=None;KWEIGHT_dn_send=None; 
   ...
   dEIGVAL = zeros((NK_proc,NBANDS),dtype=float)
   ...
   comm.Scatter(dEIGVAL_send, dEIGVAL, root=0)
   ...
   evalHF, evecHF, evalHF_dn, evecHF_dn = HF_eigsystem(KPTS,TOT_NKPTS,NBANDMAX,EIGVAL,BAND_WIN,WANU,SigMdc,SigMdc_dn)

   comm.Allgather(evalHF, evalHF_reciv)
   comm.Allgather(evalHF_dn, evalHF_dn_reciv)
   ...
   for i in range(mu_iter):
      ...
      TET_KWEIGHT_sum=zeros((TOT_NKPTS,NBANDMAX),dtype=float)
      TET_KWEIGHT=zeros((TOT_NKPTS,NBANDMAX),dtype=float)
      TET_KWEIGHT=Compute_TETRA(mu,TOT_NKPTS,NBANDMAX,tetkptr,tet_idx,evalHF_dn_reciv,TET_KPTS,ntet)
      comm.Reduce(TET_KWEIGHT,TET_KWEIGHT_sum,op=MPI.SUM)
      if rank==0: KWEIGHT_dn_send[:TOT_NKPTS,:]=TET_KWEIGHT_sum[:,:]

      comm.Scatter(KWEIGHT_send, KWEIGHT, root=0)
      comm.Scatter(KWEIGHT_dn_send, KWEIGHT_dn, root=0)
      totN=Compute_totN_HF2(KPTS,TOT_NKPTS,BAND_WIN,KWEIGHT)
      totN_dn=Compute_totN_HF2(KPTS,TOT_NKPTS,BAND_WIN,KWEIGHT_dn)
      totN=comm.allreduce(totN,op=MPI.SUM)
      totN_dn=comm.allreduce(totN_dn,op=MPI.SUM)
      totN=(totN+totN_dn)/TOT_NKPTS
   ...
   DM,MOM,EKIN = Ksum_HF(KPTS,TOT_NKPTS,NBANDMAX,NWANN,EIGVAL,BAND_WIN,WANU,evalHF,evecHF,KWEIGHT,evalHF_dn,evecHF_dn,KWEIGHT_dn)
   DM,MOM,dEKIN = Ksum_HF(KPTS,TOT_NKPTS,NBANDMAX,NWANN,dEIGVAL,BAND_WIN,WANU,evalHF,evecHF,KWEIGHT,evalHF_dn,evecHF_dn,KWEIGHT_dn)
   DM_sum=zeros(shape(DM))
   MOM_sum=zeros(shape(MOM))
   EKIN=comm.allreduce(EKIN,op=MPI.SUM)
   comm.Allreduce(DM,DM_sum,op=MPI.SUM)
   comm.Allreduce(MOM,MOM_sum,op=MPI.SUM)
   
   if rank==0:
      EKIN=EKIN/TOT_NKPTS
      DM_sum=DM_sum/TOT_NKPTS
      MOM_sum=MOM_sum/TOT_NKPTS
      #print DM_sum, MOM_sum; exit()
      Nd,EPOT=Compute_EPOT(DM_sum,MOM_sum,ncor_orb,norb,atm_idx,U,Up,Jh)
      
      HF_iter.write( '%12.6f %12.6f %12.6f %12.6f %12.6f %12.6f %12.6f %12.6f %12.6' % (mu, totN, Nd[0], Nd[-1], EKIN, EPOT, SigMdc[0], SigMdc[-1], dEKIN) )
      HF_iter.write('\n')
      HF_iter.flush()
