...
#   if ncor_orb/norb!=len(U): print "ncor_orb is not consistent with U"; exit()
   ...
   if rank==0:
      ...
      EIGVAL_send=None; dEIGVAL_send=None; WANU_send=None; KWEIGHT_send=None;KWEIGHT_dn_send=None; 
...
#   savetxt('DMFT_mu.out',[mu])
   ...
   DM,dEKIN = Ksum_HF(KPTS,TOT_NKPTS,NBANDMAX,NWANN,dEIGVAL,BAND_WIN,WANU,evalHF,KWEIGHT)
...
#      Nd,EPOT=Compute_EPOT(DM_sum,MOM_sum,ncor_orb,norb,atm_idx,U,Up,Jh)
      ...
      HF_iter.write( '%12.6f%12.6f%12.6f%12.6f%12.6f%12.6f' % (mu, totN, sum(DM_sum[:n_orbs]), sum(DM_sum[n_orbs*(Natom-1):n_orbs*Natom]), EKIN, dEKIN) )
      HF_iter.write('\n')
      HF_iter.flush()
