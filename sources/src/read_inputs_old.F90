...
contains
  ...
  subroutine Read_wan_chk()
    ...
    logical :: have_disentangled, iffile
    ...
    complex(kind=dp),allocatable:: u_matrix_opt(:,:,:), u_matrix(:,:,:)
    complex(kind=dp),allocatable:: UOPTU(:,:), UOPTU2(:,:)
    ...
    if (.not. allocated(band_win)) then
      allocate (band_win(2,num_kpts), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating band_win in Read_wan_chk')
    endif
    num_band_max=1
    DO nkp=1,num_kpts
  ...
  subroutine Read_wan_amn()
    ...
    if (iffile.eqv. .false.)then
       ...
       do i=1,num_kpts
         do j=1,num_wann
           do k=1,num_bands ! num_tot_bands
  ...
  subroutine Read_dmft_params()
    ...
    implicit none
    character(len=1) :: temp_line
    integer :: ierr, i,j,k
    logical :: iffile

    qx=2;qy=2;qz=2;n_elec=1.0_dp*num_wann; noms=400; mu_iter=100
    ...
    if (ierr /= 0) call io_error('Error Reading header in dmft_params.dat file')
    if (.not. allocated(sym_idx)) then
      allocate (sym_idx(nspin,n_atoms,n_orbs), stat=ierr)
...
!    logical :: iffile
    ...
    if (ierr /= 0) call io_error('sig.inp file is missing or has errors')
    read(20,*,iostat=ierr) (sharp, i=1,3), nom, norb_loc
    !write(*,*) sharp, nom, norb_loc
    if (ierr /= 0) call io_error('Error Reading header in sig.inp file')
    if (.not. allocated(Sigma)) then
      allocate (Sigma(norb_loc,nom), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating Sigma in Read_sig_inp')
    endif
    if (.not. allocated(om)) then
      allocate (om(nom), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating om in Read_sig_inp')
    endif
    if (.not. allocated(Sigoo)) then
      allocate (Sigoo(norb_loc), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating Sigoo in Read_sig_inp')
    endif
    if (.not. allocated(sig_loc)) then
      allocate (sig_loc(2*norb_loc+1), stat=ierr)
    ...
    do i=1,nom
      sig_loc=0.0_dp
      read(20,*,iostat=ierr) (sig_loc(j),j=1,2*norb_loc+1)
      if (ierr /= 0) call io_error('Error Reading Sigma in sig.inp file')
      om(i)=sig_loc(1)
      do j=1,norb_loc
        Sigma(j,i)=dcmplx(sig_loc(j*2),sig_loc(j*2+1))
      enddo
    enddo
    do i=1,nom
      do j=1,norb_loc
        Sigma(j,i)=Sigma(j,i)+Sigoo(j)!-Sigma(j,nom)
      enddo
    enddo
    if (allocated(sig_loc)) deallocate (sig_loc)
    ncor_orb=norb_loc

  end subroutine Read_sig_inp_real

  subroutine Read_sig_inp()
    use constants, only: dp,cmplx_0
    use io, only: io_error, io_file_unit, stdout
...
!    use io, only: io_error, io_file_unit, &
!      io_date, io_time, io_stopwatch
!    use comms, only: on_root, comms_bcast
!
!    implicit none
!
...
!    !                                                                  !
!    !===================================================================
!    use constants, only: bohr, eps6, cmplx_i
!    use utility, only: utility_recip_lattice, utility_metric
...
!    use io, only: io_date, w90_version
!    use constants, only: bohr_version_str, constants_version_str1, constants_version_str2
!    implicit none
!
...
!    !=====================================================!
!
!    use utility, only: utility_cart_to_frac, utility_lowercase
!    use io, only: io_error
!
!    implicit none
!
...
!    !===================================!
!
!    use constants, only: bohr, eps6, eps2
...
!
!    use constants, only: dp, cmplx_0, cmplx_i, twopi
!    use io, only: io_error, io_file_unit, io_date, io_time, &
!      io_stopwatch
!    use comms, only: comms_bcast, on_root
!
!    integer :: ierr
!
